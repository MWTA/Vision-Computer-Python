...
def randomIDImg(size):
    data = '0123456789abcdefghijlmnopqrstuwvxz'
    code = ''
...
for i in filenames:
    ...
    scipy.misc.imsave('../src/img/output/'+id_img+'_img_cannynewgnlbeforeminus.jpg', gnl)
    ...
    '''
    img = cv2.imread('../src/img/output/'+id_img+'_img_cannynewout.jpg')
    mask = np.zeros_like(img)
    rows, cols,_ = mask.shape

    mask = cv2.ellipse(img, center=(rows/2, cols/2), axes=(70,100), angle=0, startAngle=0, endAngle=360, color=(0,255,0), thickness=1)

    # Bitwise AND operation to black out regions outside the mask
    result = cv2.bitwise_and(img, mask)

    scipy.misc.imsave('../src/img/output/result/'+id_img+'_img_detection.jpg', result)
    
    showImg('Step 05 - Template Matching', result)
